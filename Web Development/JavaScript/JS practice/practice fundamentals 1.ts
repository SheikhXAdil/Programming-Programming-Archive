...
const objectCompare = (obj:object, compareObj:object):boolean => {
...
const itemFilter = (arr:string[], ...args:string[]):string[] => {
    arr = arr.filter((str) => {
...
const indexValExtraction = (arr:string[], args:number[]) => {
...
const accordingReduce = (args:(number[]), comparator = (a:(number), b:(number)) => a - b):number => {
...
const accordingCheck = (args:(number[]), comparator = (a:(number)) => a>5):boolean => {
...
const accordingGrouping = (arr1:number[] , arr2:(number[]) , comparator = (a:(number)) => a>5):void => {
    let group1:number[] = []
    let group2:number[] = []
