    ...
    for (let i = 0; i < strArray.length; i++) {
        if (str[i] === "*") {
            for (let j = 0; j < 10; j++) {
                ...
                let numberString = parseInt(strArray.join(""));
                if (numberString % 3 === 0) {
...
let lowerTriangularMatrixCheck = (matrix) => {
    for (let i = 0; i < matrix.length; i++) {
        for (let j = 0; j < matrix[i].length; j++) {
...
let bitFormReversion = (num) => {
    num = Math.abs(num);
    let bitForm = "00000000" + num.toString(2);
    bitForm = bitForm.slice(-8).split("").reverse().join("");
    let reversedBitNum = parseInt(bitForm, 2);
...
let alphabetReverser = (str) => {
    let result = [];
    for (let i = 0; i < str.length; i++) {
        let char_order = str.charCodeAt(i) - 'a'.charCodeAt(0), change_char = 25 - char_order + 'a'.charCodeAt(0);
...
let evenLengthArraySwapper = (arr) => {
